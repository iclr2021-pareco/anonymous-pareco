...
#            print(self.filter_ranks[key], l*self.cost_map[key])
    ...
    def prune_one_lowest_ranking_filters(self):
    ...
    def one_shot_lowest_ranking_filters_l0(self, target):
        # Consolidation of chained channels
        # Use the maximum rank among the chained channels for the criteria for those channels
        # Greedily pick from the lowest rank.
        # 
    ...
    def prune_one_filter(self, original_dist, masking=False):
        prune_targets = self.get_pruning_plan_one_filter(progressive=(not masking), get_segment=True)
        layers_pruned = {}
    ...
    def pruning_with_transformations(self, original_dist, perturbation, target, masking=False):
        ...
        prune_targets, total_pruned_importance = self.get_pruning_plan(target, progressive=(not masking), get_segment=True)
        layers_pruned = {}
        for layer_index, filter_index in prune_targets:
            if layer_index not in layers_pruned:
                layers_pruned[layer_index] = 0
        ...
        return total_pruned_importance
    ...
    def simulate_and_count_flops(self, layer_budget, mem=False):
        tmp_in_channels = dict(self.conv_in_channels)
        tmp_out_channels = dict(self.conv_out_channels)
        for layer in range(layer_budget.shape[0]):
            tmp_out_channels[layer] = layer_budget[layer]
            ...
            if next_conv_idx:
                for i in next_conv_idx:
                    next_conv = self.activation_to_conv[i]
                    if (next_conv.groups != next_conv.out_channels or next_conv.groups != next_conv.in_channels):
                        tmp_in_channels[i] = layer_budget[layer]

        cost = 0
        if not mem:
            for key in self.cost_map:
                cost += self.cost_map[key]*tmp_in_channels[key]*tmp_out_channels[key]/self.group[key]
            cost += tmp_out_channels[key]*self.num_cls
        else:
            for key in self.cost_map:
                input_mem = self.imap_size[key][0]*self.imap_size[key][1]*tmp_in_channels[key]
                output_mem = self.omap_size[key][0]*self.omap_size[key][1]*tmp_out_channels[key]
                weight_mem = tmp_in_channels[key] * tmp_out_channels[key] * self.cost_map[key] / (self.omap_size[key][0] * self.omap_size[key][1])
                res_mem = 0
                if key in self.extra_mem:
                    l = self.extra_mem[key]
                    res_mem = self.omap_size[l][0]*self.omap_size[l][1]*tmp_out_channels[l]
                cost = max(cost, input_mem+output_mem+weight_mem+res_mem)
            cost = max(tmp_out_channels[key]*self.num_cls, cost)
        ...
        for layer in sorted(self.filter_ranks.keys()):
            ...
            for k in current_chain:
                if not k in filters_to_prune_per_layer:
                    ...
                    valid_ind = tbp[tbp < cur_layer_size][:(cur_layer_size-int(layer_budget[k]))]
    ...
    def get_pruning_plan_one_filter(self, progressive=True, get_segment=False):
        if not self.quota:
            self.quota = {}
            if self.safeguard == 0:
                for k in self.filter_ranks:
                    self.quota[k] = int(self.filter_ranks[k].size(0)) - 2
            else:
                for k in self.filter_ranks:
                    self.quota[k] = np.minimum(int(np.floor(self.filter_ranks[k].size(0) * (1-self.safeguard))), int(self.filter_ranks[k].size(0)) - 2)
        filters_to_prune = self.prune_one_lowest_ranking_filters()
    ...
    def get_pruning_plan(self, num_filters_to_prune, progressive=True, get_segment=False):
        if not self.quota:
            ...
            if self.safeguard == 0:
                ...
                for k in self.filter_ranks:
                    self.quota[k] = np.minimum(int(np.floor(self.filter_ranks[k].size(0) * (1-self.safeguard))), int(self.filter_ranks[k].size(0)) - 2)
        if self.resource_type == 'filter':
            filters_to_prune = self.one_shot_lowest_ranking_filters_l0(num_filters_to_prune)
        else:
            filters_to_prune = self.one_shot_lowest_ranking_filters(num_filters_to_prune)

        filters_to_prune_per_layer = {}
        total_pruned_importance = 0
        for (l, f, r) in filters_to_prune:
            total_pruned_importance += r
            if l not in filters_to_prune_per_layer:
                filters_to_prune_per_layer[l] = []
            filters_to_prune_per_layer[l].append(f)
        ...
        return filters_to_prune, total_pruned_importance
